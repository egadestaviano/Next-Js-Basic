  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="mb-12">
          <h2 className="text-3xl font-bold mb-6 text-blue-700">Semua Produk</h2>
          {products.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {products.map((prod) => (
                <div key={prod.id} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow">
                  <h3 className="text-2xl font-semibold mb-2 text-blue-600">{prod.name}</h3>
                  <p className="text-gray-600 mb-2">Kategori: {categories.find((c) => c.id === prod.category)?.name || "-"}</p>
                  <p className="text-lg font-bold text-blue-600 mb-4">{prod.price}</p>
                  <div className="flex space-x-2">
                    <button onClick={() => handleAddToCart(prod)} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                      Add to Cart
                    </button>
